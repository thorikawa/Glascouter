...
Mat getPreprocessedFace(Mat &srcImg, int desiredFaceWidth, int desiredFaceHeight, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, bool doLeftAndRightSeparately, Rect *storeFaceRect, Point *storeLeftEye, Point *storeRightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
    // Use square faces.
    ...
    if (faceRect.width > 0) {
        ...
        if (leftEye.x >= 0 && rightEye.x >= 0) {
            ...
            */
            ...
            // Do the best even if eyes are not detected
        	Mat dstImg;
            Mat filtered = Mat(gray.size(), CV_8U);
            equalizeHist(gray, gray);
            bilateralFilter(gray, filtered, 0, 20.0, 2.0);
            resize(gray, dstImg, Size(desiredFaceWidth, desiredFaceHeight));
        	return dstImg;
        }
    }
    return Mat();
